...
<form id="${ id }" action="${ url }" method="post"<% if (config.noDecoration) { %> style="display: inline"<% } %> status="true" >
...
<% if (config.cancelLabel) { %>
	<button type="reset" onclick="publish('${ id }.reset'); ${ config.cancelFunction }()">${ config.cancelLabel }</button>
<% } %>
    ...
    <script>
    	jq(function() {
	        jq('#${ id }').submit(function(e) {
	            e.preventDefault();
	             var form = jq("#${id}");
	           	if(form.attr("status")=="false"){
	          	 	alertify.error("In progress....");
	           	    return;
	           	}
	           	form.attr("status","false");
